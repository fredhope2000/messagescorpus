...
neighborhood <- function(index,length=15)
{
	if (length < 1) { length <- 15 }
	catTables(table=1,start=(index-3),stop=(index+length-4))
}
...
cleanUpTable <- function(table)
{
	...
	table <- table[!is.na(table)] #this is more efficient and seems to be equivalent to the commented-out code below
#	i <- length(table)
#	while (i >= 1)
#	{
#		if (is.na(table[i]))
...
myName <- "Fred" #shows how your name displays in the conversation tables
myEmail <- "fredhope2000@gmail.com" #your iCloud email shows up sometimes in the message logs, so put it here so things get parsed correctly
setwd("/Users/fred/messagescorpus/") #directory containing the message log files to be inputted
excludeNames <- c("Fred Hope") #gets messy when we use conversations with ourselves, debug this later
...
duplicateNames <- list("charley@julesallen.net","Mark","mmyslin@me.com","mmyslin@yahoo.com","mmyslin@icloud.com","+1 (805) 680-4273","MaryAnne (gp+)","callenrandolph@gmail.com","lucy2424@sbcglobal.net","Mom","Richard (gp+)","Daniel Benjamin", "Ana Banana", "anacuevas28@gmail.com")
names(duplicateNames) <- c("Charley","Mark Myslin","Mark Myslin","Mark Myslin","Mark Myslin","Caryn Parker", "MaryAnne Contreras","Cameron (gp)","Louise Hope","Louise Hope","Richard (sb)","Dan", "Ana", "Ana")
oppressScanMessages <- TRUE
